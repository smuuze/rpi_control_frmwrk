...
#define TEST_CASE_ID_MESSAGE_RECEIVED_TO_LONG			8
...
u8 counter_MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL = 0;
...
static void unittest_mqtt_counter_reset(void) {
	...
	counter_MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL = 0;
...
u8 mqtt_enqeue_message(MQTT_INTERFACE* p_mqtt_interface, const char* p_msg_from) {
	...
	switch (UT_GET_TEST_CASE_ID()) {
		...
		case TEST_CASE_ID_MESSAGE_TO_SEND :
			...
			memset(unittest_MSG_TO_SEND, '\0', MAX_MSG_LENGTH);
			if (MAX_MSG_LENGTH < strlen(p_msg_from)) {
				memcpy(unittest_MSG_TO_SEND, p_msg_from, MAX_MSG_LENGTH);
			} else {
...
static void unittest_ENQEUE_TX_MESSAGE_FAILED_CALLBACK(const void* p_argument) {

	(void) p_argument;
	DEBUG_PASS("unittest_ENQEUE_TX_MESSAGE_FAILED_CALLBACK()");
	counter_MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL += 1;
}
...
SIGNAL_SLOT_INTERFACE_CREATE_SLOT(MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL, UNITTEST_MQTT_ENQEUE_TX_MESSAGE_FAILED_SLOT,   unittest_ENQEUE_TX_MESSAGE_FAILED_CALLBACK)
...
static void UNITTEST_mqtt_interface_init(void) {
	...
	{	
		...
		UNITTEST_MQTT_ENQEUE_TX_MESSAGE_FAILED_SLOT_connect();
		...
		UT_CHECK_IS_EQUAL(counter_MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL, 0);
	}
	UT_END_TEST_CASE()
}
...
static void UNITTEST_mqtt_interface_msg_to_send(void) {
	...
	{	
		...
		DEBUG_TRACE_STR(TEST_CASE_MSG_REPORT_TO_SEND_STRING, "UNITTEST_mqtt_interface_msg_to_send()");
		MQTT_MESSAGE_TO_SEND_SIGNAL_send(TEST_CASE_MSG_REPORT_TO_SEND_STRING);
		...
		UT_COMPARE_STRING(unittest_MSG_TO_SEND, TEST_CASE_MSG_REPORT_TO_SEND_STRING);
		UT_CHECK_IS_EQUAL(counter_MQTT_CONNECTION_FAILED_SIGNAL, 0);
		UT_CHECK_IS_EQUAL(counter_MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL, 0);
	}
	UT_END_TEST_CASE()
...
static void UNITTEST_mqtt_interface_received_message_to_long(void) {
	
	UT_START_TEST_CASE("MQTT_INTERFACE_RECEIVED_MESSAGE_TO_LONG")
	{	
		UT_SET_TEST_CASE_ID(TEST_CASE_ID_MESSAGE_RECEIVED_TO_LONG);

		unittest_reset_counter();

		char message_to_long[MQTT_INTERFACE_MAX_MSG_LENGTH + 10];
		memset(message_to_long, '\0', sizeof(message_to_long));
		memset(message_to_long, 'A', sizeof(message_to_long) - 1);

		UNITTEST_TIMER_start();
		printf(" - TEST-CASE start time (ms) : %d (%x04)\n", UNITTEST_TIMER_start_time(), UNITTEST_TIMER_start_time());

		u8 message_send = 0;
		u16 message_Send_timeout_ms = 100;

		while (UNITTEST_TIMER_is_up(250) == 0) {

			if (message_send == 0 && UNITTEST_TIMER_is_up(message_Send_timeout_ms)) {
				message_send = 1;
				MQTT_MESSAGE_TO_SEND_SIGNAL_send(message_to_long);
			}
		...
		UT_CHECK_IS_GREATER(counter_MQTT_TX_MSG_PENDING_CALLBACK, 0);
		UT_CHECK_IS_EQUAL(counter_MQTT_CONNECTION_FAILED_SIGNAL, 0);
		UT_CHECK_IS_EQUAL(counter_MQTT_ENQEUE_TX_MESSAGE_FAILED_SIGNAL, 1);
	}
	UT_END_TEST_CASE()
}
...
int main(void) {
	...
	UT_START_TESTBENCH("Welcome the the UNITTEST for mqtt-interface v1.0")
	{
		...
		UNITTEST_mqtt_interface_received_message_to_long();
