...
#define IO_CONTROLLER_BUILD_OUTPUT(name, pin_id)													\
																									\
...
#define IO_CONTROLLER_BUILD_INPUT(name, pin_id, idle_state)											\
																									\
	static u8 _##name##_input_id = 0;																\
																									\
	static IO_INPUT_DESCRIPTOR _name##input_pin = {													\
		0, /* u8 id; */																				\
		pin_id, /* u8 pin_id; */																	\
		0, /* u8 down; */																			\
		0, /* u8 pressed; */																		\
		...
		idle_state, /* SYSTEM_INTERFACE_GPIO_LEVEL idle_state; */									\
		0 /* struct IO_INPUT_DESCRIPTOR_T* __next_button; */										\
	};																								\
																									\
	void name##_init(void) {																		\
		_##name##_input_id  = io_input_controller_register_input(&_name##input_pin);				\
	}																								\
																									\
	u8 name##_get_state(void) {																		\
		return _name##input_pin.actual_pin_state;													\
	}																								\
																									\
	u8 name##_is_pressed(void) {																	\
		return _name##input_pin.pressed;															\
	}																								\
																									\
	u8 name##_is_down(void) {																		\
		return _name##input_pin.down;																\
	}																								\
																									\
	u8 name##_is_released(void) {																	\
		return _name##input_pin.released;															\
	}																								\
																									\
	u32 name##_get_down_time(void) {																\
		return _name##input_pin.__down_time;														\
	}
...
#define IO_CONTROLLER_BUILD_INOUT(name, pin_id)														\
																									\
	static u8 _##name##_inout_id = pin_id;															\
																									\
	void name##_drive_high(void) {																	\
		/* OUTPUT_HIGH_LEVEL  */																	\
		i_system.io.set_direction(_##name##_inout_id, GPIO_OUTPUT);									\
		i_system.io.set_level(_##name##_inout_id, GPIO_LEVEL_HIGH);									\
	}																								\
																									\
	void name##_drive_low(void) {																	\
		/* OUTPUT_LOW_LEVEL  */																		\
		i_system.io.set_direction(_##name##_inout_id, GPIO_OUTPUT);									\
		i_system.io.set_level(_##name##_inout_id, GPIO_LEVEL_LOW);									\
																									\
	}																								\
																									\
	void name##_no_drive(void) {																	\
		/* INPUT_HIGH_Z  */																			\
		i_system.io.set_direction(_##name##_inout_id, GPIO_INPUT);									\
		i_system.io.set_level(_##name##_inout_id, GPIO_LEVEL_HIGH_Z);								\
																									\
	}																								\
																									\
	...
	u8 name##_is_high_level(void) {																	\
		return gpio_driver_get_level(_##name##_inout_id) == GPIO_LEVEL_HIGH ? 1 : 0;				\
	}																								\
																									\
	u8 name##_is_low_level(void) {																	\
		return gpio_driver_get_level(_##name##_inout_id) == GPIO_LEVEL_LOW ? 1 : 0;					\
	}																								\
