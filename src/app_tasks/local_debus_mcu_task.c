...
#define DEBUS_HOST_COM_UPDATE_TIMEOUT()	__host_com_time = i_system.time.now_u16();
#define DEBUS_HOST_COM_IS_TIMEOUT()	(i_system.time.isup_u16(__host_com_time, DEBUS_HOST_COM_TIMEOUT_MS) ? 1 : 0)
...
void debus_task_run(void) {
	...
	{	
		if (!debus_msg_ready()) {
			...
			if (i_system.time.isup_u16(__host_com_time, DEBUS_HOST_COM_TIMEOUT_MS)) {
				DEBUS_SET_STATUS(DEBUS_STATUS_HOST_COM_TIMEDOUT);
			}
			
		} else if (!debus_tx_busy()) {
